...
const newArticle = Array(picArray.length)
...
for (let i = 0; i < newArticle.length; i++) {
  ...
  newArticle[i].addEventListener("click", function () {
    displayModal(i)
  })
  ...
  titleEl.textContent = picArray[i].title
  imgEl.src = picArray[i].image.medium
  imgEl.alt = picArray[i].title
  figcaptionEl.textContent = picArray[i].caption
  pEl.textContent = picArray[i].description

  newArticle[i].appendChild(titleEl)
  newArticle[i].appendChild(figureEl)
  newArticle[i].appendChild(pEl)
  figureEl.appendChild(imgEl)
  figureEl.appendChild(figcaptionEl)

  pictures.appendChild(newArticle[i])
}

function displayModal(i) {
  modal_img.src = picArray[i].image.large
  modal_img.alt = picArray[i].title
